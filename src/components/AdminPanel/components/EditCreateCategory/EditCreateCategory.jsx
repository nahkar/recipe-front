...
export default function EditCreateCategory({ history }) {
  ...
  const sendCategory = async (category) => {
    if (category) {
      id
        ? await api.user.editCategory(id, { title: category.trim() })
        : await api.user.createCategory({ title: category.trim() });
  ...
  return (
    <CreateCategoryStyled.Wrapper>
      ...
      <CreateCategoryStyled.NameInput>
        ...
        <Input
          margin='30px'
          isError={category.length >= MAX_LENGTH}
          type='text'
        ...
        />
        {category.length >= 20 && (
          <CreateCategoryStyled.WarningMessage>
            Сategory must have a maximum of 20 characters{' '}
          </CreateCategoryStyled.WarningMessage>
        )}
